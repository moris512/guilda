classdef GSC_for_PV < component_for_farm.base

    methods
        function obj = GSC_for_PV(varargin)
        ...
        function [dx, I_tobus, V_toDClink, I_toDClink] = get_dx_VI(x, Vbus, vdc, u)
            ...
            [dx_out, Iref] = get_dx_Iref(obj, t, x_out, -real(PQ), -imag(PQ), u_out);
        ...
        function x_st = set_equilibrium(obj, V, I, vdc_p, idc_p)
            obj.set_nxu;
            Idq = I;
            Pst = V(1)*Idq(1) + V(2)*Idq(2);
            Qst = V(2)*Idq(1) - V(1)*Idq(2);
            x_st{1} = Idq;
            x_st{2} = obj.Inner_loop_controller.set_equilibrium(Idq);
            x_st{3} = obj.Outer_loop_controller.set_equilibrium(Idq, -Pst, -Qst);
            x_st = vertcat(x_st{:});
        end
