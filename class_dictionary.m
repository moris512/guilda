function class_dictionary(class_name)
            ...
            if  ~list(idx).isdir
                ...
                if numel(dir_list)~=0
                    for i = numel(dir_list):-1:1
                        if dir_list{i}(2)=='+'
                            filename = [dir_list{i}(3:end),'.',filename];
                        end
                    end
                end
                ...
                if ~any(data{:,'filename'}==filename)
                    try
                        ...
                        disp("Unable to determine parent class: "+filename)
                    end
                    if numel(superclass_list)>0 && any(strcmp(superclass_list,class_name))
                        ...
                        data = vertcat(data,table(filename,superclass));
                    end
                end
            else
